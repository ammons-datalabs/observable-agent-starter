...
pytestmark = pytest.mark.skipif(
    not os.getenv("OPENAI_API_KEY"), reason="DeepEval requires OPENAI_API_KEY"
)
...
def test_profile():
    return InfluencerProfile(
        ...
        publishing_cadence={"planned_per_week": 3, "actual_last_28_days": 10},
    )
...
def test_relevancy_to_request(generator, test_profile):
    ...
    ideas = generator(
        profile=test_profile, request="Generate ideas about AI productivity tools for creators"
    )

    actual_output = "\n".join([f"{i.title}: {i.summary}" for i in ideas])
...
def test_pillar_adherence(generator, test_profile):
    ...
    for idea in ideas:
        assert (
            idea.pillar in test_profile.content_pillars
